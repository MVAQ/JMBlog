...
<?php //dd($mspost); ?>
        <div class="thumbnail">
            <!-- post title -->
            <h1 class="post-title"><a href="#">{{ $mspost->title }}</a></h1>
            <!-- post featured -->
            <div class="image-wrap">
                <img src="{{ asset(env('THEME')) }}/images/articles/{{ $mspost->imgs->max}}" alt="00212" title="00212" />
            </div>
            <p class="date">
        ...
        <div class="meta group">
            <p class="author"><span>by <a href="#" title="{{ $mspost->title }}" rel="author">{{ $mspost->user->name }}</a></span></p>
            <p class="categories"><span>In: <a href="{{ route ('mspostsCat',['cat_alias'=>$mspost->categorie->alias]) }}" title="View all posts in {{  $mspost->categorie->title }}" rel="category tag">{{ $mspost->categorie->alias }}</a></span></p>
            <p class="comments"><span><a href="#comments" title="Comment on This is the title of the first article. Enjoy it.">{{ count($mspost->comments)?  count($mspost->comments):"0" }}  {{ Lang::choice('ru.comments',count($mspost->comments)) }}</a></span></p>
        </div>
        <!-- post content -->
    ...
    <div id="comments">
        <h3 id="comments-title">
            <span>{{ count($mspost->comments) }}</span>  {{ Lang::choice('ru.comments',count($mspost->comments)) }}
        </h3>

        @if(  count($mspost->comments)>0 )

        @set($com, $mspost->comments->groupBy('parent_id'))
        ...
        <div id="respond">
            ...
            <form  action="{{ route('comment.store') }}" method="post" id="commentform">
                ...
                <p class="form-submit">
                    ...
                    <input name="comment_post_id" type="hidden" id="comment_post_id" value="{{ $mspost->id }}" />
                    <input name="comment_parent" type="hidden" id="comment_parent" value="" />
